...
int main() {
    ...
    // Cache de resultados: <nombre, <permutacion, costo>>
    vector<pair<string, pair<vector<int>, double>>> resultados;

    while (true) {
        system("cls"); // Windows; en Linux/macOS usar "clear"
        ...
        if (opcion == "1") {
            ...
            try {
                ...
                archivoActual = ruta;
                resultados.clear();
                cout << "\n✔ " << tablones.size() << " tablones cargados desde: " << ruta << "\n";
            ...
            if (opcion == "9") {
                ...
                if (resFB.second == resPD.second) {
                    cout << "✔ FB y PD coinciden → solucion OPTIMA encontrada.\n";
                    if (resV.second == resFB.second) {
                        cout << "✔ El Voraz tambien encontro el optimo.\n";
                    } else {
                        cout << "⚠ El Voraz NO encontro el optimo (costo: "
                             << resV.second << " vs " << resFB.second << ").\n";
                    }
                } else {
                    cout << "⚠ Diferencia entre FB y PD. Revise la implementacion.\n";
...
vector<Tablon> leerArchivo(const string& ruta) {
    ...
    // Limpiar posibles BOM o caracteres raros
    ...
    while (getline(archivo, linea)) {
        ...
        // Reemplazar comas por espacios para parsing uniforme
        for (char& c : linea) {
            if (c == ',') c = ' ';
        }
...
void verificarCosto(const vector<Tablon>& tablones, const vector<int>& permutacion, double costoEsperado) {
    ...
    for (size_t paso = 0; paso < permutacion.size(); paso++) {
        ...
        if (rp == tiempo) {
            ...
            // Caso 2: dentro del tiempo de supervivencia
            costoPaso = 2 * (ts - (tiempo + tr));
            rama = "(ts - tr) >= t  → Costo = 2 * (ts - (t + tr)) = 2 * ("
                   + to_string(ts) + " - (" + to_string(tiempo) + " + " + to_string(tr) + "))"
                   + " = " + to_string(costoPaso);
        } else {
            // Caso 3: fuera de tiempo, penalización por prioridad
...
void guardarSalida(const pair<vector<int>, double>& res, const string& ruta) {
    ...
    // Formato 3.4.2: primera línea el costo, luego n líneas con los índices
...
bool compararResultados(const vector<pair<string, pair<vector<int>, double>>>& resultados) {
    ...
    if (todosOk) {
        cout << "✔ TODOS los algoritmos coinciden en el mismo costo: "
             << fixed << setprecision(0) << costoRef << "\n";
    } else {
        cout << "⚠ Los algoritmos NO coinciden. Revise diferencias arriba.\n";
    }
